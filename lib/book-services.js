...
class BookServices {
        ...
        return new Promise((resolve) => {
            ...
            HtmlProcessor.extractImages(section.url, section.content)
                .then((extractedImages) => {
                    ...
                    log.info(`filteredSection.images: ${filteredSection.images}`)
    ...
    */
    ...
    static convertSectionsContent(book) {
        ...
        const sections = book.getSections();
        log.info(`convertSectionsContent`);
        return new Promise((resolve) => {
    ...
    */
    ...
    /*
        Step 8: Schedule Clean todo hank 这里为什么没有正确执行?
    */

    static scheduleClean(book) {
        log.info(`scheduleClean: ${book}`);
        setTimeout(() => {
            log.info(`scheduleClean: ${book.getEpubPath()}`);
            book.deleteFiles();
