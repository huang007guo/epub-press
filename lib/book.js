...
class Book {
    ...
    static getEpubWriter(metadata) {
        ...
        metadata1.cover = metadata.coverPath || Book.DEFAULT_COVER_PATH;
        // eslint-disable-next-line
        const nodepub = require('nodepub');
        const document = nodepub.document(metadata1, Book.getToc);
        return document;
    ...
    writeEpub() {
        try {
            ...
            log.exception('writeEpub ERROR')(e);
            throw e;
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            // log.info(`_ebook: ${JSON.stringify(this._ebook)}`);
            (async () => {
                try {
                    await this._ebook.writeEPUB(Book.DEFAULT_EBOOK_FOLDER, this.getMetadata().id);
                    // this.deleteAssets();
                    resolve(this);
                } catch (e) {
                    log.exception('writeEpub')(e);
                    reject(e);
                }
            })();
...
Book.DEFAULT_METADATA = {
    author: 'EpubPress-Hank-Mod',
    description: 'Built using https://blog.csdn.net/huang007guo',
    genre: 'Unknown',
    language: 'zh',
