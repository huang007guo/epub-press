...
class StylingService {
    ...
    static stringSize(str) {
        ...
        for (let i = 0; i <= strS.length; i += 1) {
            if (strS[i]) {
                // log.info(`StylingService.isAscii,
                // ${StylingService.isAscii(strS[i])}, ${strS[i]}`);
                sumWidth += (StylingService.isAscii(strS[i]) ?
                    StylingService.CHAR_ASCII_WIDTH : StylingService.CHAR_UNASCII_WIDTH);
            }
        }
        return sumWidth;
    }
...
StylingService.FONT_PATH = Config.JIMP_FONT_PATH || Jimp.FONT_SANS_64_BLACK;
StylingService.MAX_X = 816;
StylingService.MIN_Y = 690;
StylingService.MARGIN = 7;
StylingService.LINE_WIDTH = StylingService.MAX_X - StylingService.MARGIN * 2;
StylingService.LINE_HEIGHT = 80;
StylingService.MAX_LINES = 4;
StylingService.CHAR_ASCII_WIDTH = 33;
// 定义中文字体(不是ASCII)的宽度
StylingService.CHAR_UNASCII_WIDTH = 63;
