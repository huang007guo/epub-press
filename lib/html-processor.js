    ...
    */
    ...
    static _filterDownloadedImages($, imgStatuses) {
        ...
        $('img').each((index, elem) => {
            ...
            if (result && !result.error && result.path) {
                // 这里在window系统下是无法使用的,window下文件地址:
                // G:\\xampp\\htdocs\\my\\epub-press\\tmp\\images\\r1gZQlxBYt.png
                // const bookPath = result.path.replace(/.*(images\/.*)/, '../$1');
                let lastIndexOf = result.path.lastIndexOf('\\');
                // 这不是window
                if (lastIndexOf < 0) {
                    lastIndexOf = result.path.lastIndexOf('/');
                }
                const bookPath = `../images/${result.path.slice(lastIndexOf + 1)}`;
                ...
                log.info(`$(elem):${$(elem).attr('src')}`);
                downloadedImages.push(result.path);
            } else {
                log.exception(`download fail, result: ${JSON.stringify(result)}`);
                $(elem).remove();
            }
        });
    ...
    static extractImages(rootUrl, html) {
        ...
        const imgMap = selectedSources.reduce((map, src) => {
            map[src] = HtmlProcessor.absolutifyUrl(rootUrl, src);
            return map;
        }, {});
        ...
        return new Promise((resolve) => {
            HtmlProcessor.downloadImages(imgMap)
                .then((imgStatuses) => {
                    // log.info(`imgStatuses: ${JSON.stringify(imgStatuses)}`);
    ...
    static getImageSources($) {
        ...
        $('img').each((index, elem) => {
            ...
            if (src && src.indexOf('data:') === 0) {
                ...
                // 对src 中的 url做一定的处理
                // let newSrc = encodeURIComponent(src);
                // newSrc = newSrc.replace(/%2F/g, '/');
    ...
    static absolutifyUrl(root, url) {
        ...
        const normalUrl = url.slice(0, 2) === '//' ? `${rootParsed.protocol}//${url}` : url;
        ...
        if (parsed.host) {
            ...
            const dir =
                rootParsed.path.slice(-1) === '/' ? rootParsed.path : path.dirname(rootParsed.path);
            log.info(`absolutifyUrl dir: ${dir} rootParsed: ${JSON.stringify(rootParsed)} normalUrl: ${normalUrl}, parsed: ${JSON.stringify(parsed)}`);

            // 如果是/开头直接加host即可
            if (normalUrl.slice(0, 1) === '/') {
                absoluteUrl = (`${rootParsed.protocol}//${rootParsed.host}${normalUrl}`);
            } else {
                // 当前页面真实的路径
                let readyPath = '/';
                const pathLastIndexOf = rootParsed.path.lastIndexOf('/');
                if (pathLastIndexOf > 0) {
                    readyPath = rootParsed.path.slice(0, pathLastIndexOf + 1);
                }
                absoluteUrl = (`${rootParsed.protocol}//${rootParsed.host}${readyPath}${normalUrl}`);
            }
            // absoluteUrl = Url.resolve(root, path.resolve(dir, normalUrl));
        }
        log.info(`absolutifyUrl absoluteUrl, ${absoluteUrl}`);
        return absoluteUrl;
    }

    static downloadImages(imgMap) {
        log.info(`downloadImages imgMap: ${JSON.stringify(imgMap)}`);
